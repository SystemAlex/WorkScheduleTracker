...
import { insertEmployeeSchema, insertPositionSchema, insertShiftTypeSchema, insertShiftSchema } from "@shared/schema";
import { z } from "zod";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/shifts", async (req, res) => {
    try {
      ...
      if (month && year) {
        shifts = await storage.getShiftsByMonth(parseInt(month as string), parseInt(year as string));
      } else {
  ...
  app.get("/api/reports/employee-hours", async (req, res) => {
    try {
      ...
      res.status(500).json({ message: "Failed to generate employee hours report" });
    }
  });
