  ...
  return (
    <div className="bg-white rounded-xl shadow-md p-6">
      ...
      <div className="grid grid-cols-7 gap-px bg-neutral-200 rounded-lg overflow-hidden mb-4">
        {dayNames.map((day) => (
          <div key={day} className="bg-neutral-100 p-3 text-center text-sm font-semibold text-neutral-700">
            {day}
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-7 gap-px bg-neutral-200 rounded-lg overflow-hidden">
        {days.map((day, index) => {
          ...
          const isSelected = selectedDate && formatDate(selectedDate) === formatDate(day.date);
                ...
                {dayShifts.slice(0, 3).map((shift) => {
                  ...
                  return (
                    <div
                      ...
                      className={cn(
                        "shift-indicator",
                        `shift-${color === 'blue' ? 'morning' : 
                               color === 'green' ? 'afternoon' : 
                               color === 'orange' ? 'night' : 'special'}`
                      )}
                    >
                      <div className={cn(
                      ...
                      <span>{shift.shiftType.code}: {shift.employee.name.split(' ')[0]}</span>
                    </div>
                  );
                })}
                {dayShifts.length > 3 && (
      ...
      <div className="mt-6 bg-white rounded-xl shadow-md p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="flex items-center space-x-3">
            ...
            <span className="text-sm text-neutral-700">Mañana (M) - 6:00-14:00</span>
          </div>
          <div className="flex items-center space-x-3">
            <div className="w-4 h-4 bg-green-500 rounded-full"></div>
            <span className="text-sm text-neutral-700">Tarde (T) - 14:00-22:00</span>
          </div>
          <div className="flex items-center space-x-3">
            <div className="w-4 h-4 bg-orange-500 rounded-full"></div>
            <span className="text-sm text-neutral-700">Noche (N) - 22:00-6:00</span>
          </div>
          <div className="flex items-center space-x-3">
            <div className="w-4 h-4 bg-purple-500 rounded-full"></div>
            <span className="text-sm text-neutral-700">Especial (E) - Variable</span>
          </div>
        </div>
      </div>
    </div>
