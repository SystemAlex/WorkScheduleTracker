...
export default function Organigrama() {
    ...
    return (
      <div className="flex-1 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-sm text-neutral-500 mt-2">Cargando organigrama...</p>
        </div>
      </div>
    );
  }
  ...
  const getEmployeesForPosition = (positionName: string) => {
    return employees.filter(emp => emp.position === positionName && emp.status === 'active');
  };
          ...
          {Object.entries(departments).map(([departmentName, deptPositions]) => (
            <div key={departmentName}>
              <div className="flex items-center space-x-3 mb-4">
                ...
                <div>
                  <h3 className="text-xl font-bold text-neutral-900">{departmentName}</h3>
                  <p className="text-sm text-neutral-500">
                    {deptPositions.length} puesto{deptPositions.length !== 1 ? 's' : ''} 
                    • {deptPositions.reduce((acc, pos) => acc + getEmployeesForPosition(pos.name).length, 0)} empleado{deptPositions.reduce((acc, pos) => acc + getEmployeesForPosition(pos.name).length, 0) !== 1 ? 's' : ''}
                  </p>
                </div>
              </div>
                    ...
                    <Card key={position.id} className="border-l-4 border-l-blue-500">
                      <CardHeader className="pb-4">
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-neutral-500">
                              {positionEmployees.length} empleado{positionEmployees.length !== 1 ? 's' : ''}
                            </p>
                          </div>
                        </div>
                      </CardHeader>
                      <CardContent>
                        {position.description && (
                          <p className="text-sm text-neutral-600 mb-4">{position.description}</p>
                        )}
                        
                        {positionEmployees.length > 0 ? (
                          <div className="space-y-3">
                            <h4 className="text-sm font-semibold text-neutral-700">Empleados:</h4>
                            {positionEmployees.map((employee) => (
                              <div key={employee.id} className="flex items-center space-x-3 p-2 bg-neutral-50 rounded-lg">
                          ...
                          <div className="text-center py-4">
                            ...
                            <p className="text-sm text-neutral-500">Sin empleados asignados</p>
                          </div>
                        )}
                      </CardContent>
                    </Card>
